...
export class BookingsService {
  ...
  async list(user: AuthenticatedUser): Promise<SessionListResponse> {
    ...
    const now = Date.now();
    const alive = sessions.filter((session) => !this.isExpired(session, now));
    return {
      upcoming: alive
        .filter((session) => session.startsAt.getTime() >= now)
        .map((session) => toSessionResponse(session, this.avatarUrlOf)),
      past: alive
        .filter((session) => session.startsAt.getTime() < now)
        .reverse()
        .map((session) => toSessionResponse(session, this.avatarUrlOf)),
  ...
  async pay(
    ...
    const transitioned = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.session.updateMany({
        ...
        data: { status: SessionStatus.PAID_ESCROW, expiresAt: null },
      });
      if (updated.count === 0) {
        return false;
      }
