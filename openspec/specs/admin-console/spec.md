...
## Purpose
Admin console gathering all admin duties in one place: a user directory with account suspension, a read-only payment ledger, an analytics overview with per-currency money totals, and review moderation — admin-only and localized in all five locales.
...
The system SHALL provide admins an analytics overview reporting: user counts by role and suspended count, session counts by status, dispute counts (open, and resolved by outcome), and money totals grouped per currency in integer minor units — held, released, and refunded amounts plus platform fee revenue from released payments — along with a recent daily trend of created sessions and released amounts. Totals SHALL be computed from stored records; amounts of different currencies SHALL never be summed together.
