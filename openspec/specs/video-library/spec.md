...
## Purpose
Amateur video library for technique analysis: resumable multipart uploads direct to S3 that preserve the original file byte-identical, server-side validation and conditional playback transcode, and owner-scoped management (watch, download original, rename, delete) with a visible processing lifecycle.
