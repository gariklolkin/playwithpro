# calendar-invites Specification

## Purpose
Calendar invitations for paid sessions through a `CalendarProvider` abstraction: universal `.ics` email invites on payment success (session-room URL for online services, venue address for in-person games) and cancellation updates referencing the same event UID.
