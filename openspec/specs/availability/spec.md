...
## Purpose
Coach-owned session availability: an optional recurring weekly template materialized into concrete bookable 60-minute slots (UTC, rolling 4-week horizon), calendar-first slot management with per-slot overrides, and the public open-slot listing the booking flow consumes.
## Requirements
...
A professional SHALL manage availability on a dedicated calendar-first dashboard page with two tabs. The Calendar tab (default) SHALL show a month grid in the coach's timezone (timezone labeled, horizon noted) with per-day open/booked slot counts; selecting a day SHALL open an hour editor where one tap opens or removes a slot, booked hours are locked, and past hours are disabled. The Weekly template tab SHALL hold the recurring-template editor with an explicit dirty-state save. All copy SHALL come from the localized message catalogs.
...
- **THEN** the slot is shown under the correct local day at 18:00 with the timezone labeled

#### Scenario: One-tap slot editing
- **WHEN** a coach opens a future day and taps a free hour, then taps the same hour again
- **THEN** a slot is created and then removed, and the day's open-slot count follows

#### Scenario: Booked hours are locked
- **WHEN** a coach opens a day containing a booked slot
- **THEN** that hour is displayed as booked and offers no removal control
