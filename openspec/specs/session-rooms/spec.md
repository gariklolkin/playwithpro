...
## Purpose
Online session rooms: party-only access to an embedded Jitsi call within a join window, side-by-side attached video for video-analysis sessions, attendance logging, and clock-driven session progression from escrow to awaiting confirmation.
