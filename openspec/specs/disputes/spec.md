...
## Purpose
Dispute handling for paid sessions: the player can report a problem during the confirmation window instead of confirming, freezing escrow; admins review an evidence-backed dispute queue and resolve each dispute exactly once — releasing to the coach or refunding the player — with the outcome visible to both parties in all five locales.
