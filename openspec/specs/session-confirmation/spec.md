# session-confirmation Specification

## Purpose
Post-session completion: player confirmation atomically completes the session and releases escrow to the coach minus the platform fee, coach confirmation is recorded as dispute evidence only, and a configurable auto-confirm window (default 48h) completes unattended sessions via sweep and read-path normalization — all localized in five locales.
