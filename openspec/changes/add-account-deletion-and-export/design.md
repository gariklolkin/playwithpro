...
At request (one transaction + follow-ups): `AccountDataRequest(DELETION, SCHEDULED, scheduledFor = now + grace)`, `User.deletionScheduledFor`, revoke all refresh tokens, cancel unpaid sessions, delete availability rules and mark future `OPEN` slots `REMOVED`, withdraw a scheduled verification call (Google event deleted). Catalog, coach page, public slots and coach access to the player's clips filter on `deletionScheduledFor IS NULL AND deletedAt IS NULL`. Cancelling clears `deletionScheduledFor` and marks the request `CANCELLED`; rules and slots are not resurrected (the coach republishes) — stated in the dialog. Only a self-requested deletion can be cancelled by the user: an admin-scheduled one is refused (403) and the grace screen shows a notice instead of the button (`DeletionStatusResponse.initiatedBy`). The blockers are checked before the credential (a one-time code must not be burnt on a refused request) and again inside the request transaction after the `deletionScheduledFor` stamp — the stamp locks the user row, and `pay()` reads both parties' rows `FOR NO KEY UPDATE` inside its own transaction, so a payment either committed before the stamp (and is seen as a blocker) or waits for it and then refuses. Unpaid bookings are cancelled on both sides (the user as player and as coach).
...
`AccountErasureHook { name: string; erase(userId, ctx): Promise<void> }` registered as a multi-provider (`ACCOUNT_ERASURE_HOOKS`). Order: `credentials` (tokens, OAuth, verification tokens), `player-profile`, `pro-profile` (services deleted, bio/languages cleared, status unchanged), `availability`, `verification` (scheduled bookings cancelled, admin notes cleared), `videos` (`VideosService.purge` per row), `avatars` (`deletePrefix`), `exports` (`exports/<userId>/` deleted, keys cleared — a zip holds the whole inventory and must not outlive the account), `observability` (PostHog person delete when `POSTHOG_PERSONAL_API_KEY` is set, else `skipped`), then the tombstone. `AccountDataRequest.steps` is JSON `{ [name]: { status: 'done'|'failed'|'skipped', at, error? } }`; a run skips `done` steps, retries the rest, and the request is `FAILED` (retryable from the console only — the hourly job never re-runs a failed request, so a permanently failing hook does not loop) if any step failed, or if anything outside the hooks threw after the claim (nothing may stay `RUNNING`) — the tombstone runs only when every prior step is done. The completion email is delivered directly (not queued) to the address read before the scrub, right after the scrub commits, so a retried scrub cannot send it twice; every other email goes through the outbox with `sessionId: null`. The verification withdrawal is shared with the request path (`withdrawOpenVerification` → `SchedulingService.withdraw`), not re-implemented.
