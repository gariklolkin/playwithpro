# production-deploy — delta

## ADDED Requirements

### Requirement: Public HTTPS endpoints
The platform SHALL be reachable at `https://play-with.pro` (web, with `www` redirecting to the apex), `https://api.play-with.pro` (API), and `https://meet.play-with.pro` (Jitsi), each serving a valid Let's Encrypt certificate that renews automatically.
...
The production stack (web, api, PostgreSQL, Jitsi components) SHALL run as Kubernetes workloads on a single k3s node, declared by manifests in the repository, with the api limited to exactly one replica while playback-sync state is in-memory.
...
### Requirement: Production Jitsi over HTTPS
The Jitsi stack SHALL be served at `https://meet.play-with.pro` with WebRTC media flowing through the host's public IP (JVB on UDP 10000, coturn fallback on 3478), embedded by the session room page on its real secure origin, and its production config SHALL retain the VP8-first codec order.
