...
**Reason**: Replaced by "Token-authorized video room via provider abstraction" and "Native call UI in the session room" — the Jitsi iframe embed is retired.
**Migration**: The `embedded_jitsi` room descriptor and the `leave` endpoint are removed; clients use the `livekit` descriptor from the room endpoint and the token returned by the join action.
