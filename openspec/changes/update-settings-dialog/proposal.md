...
- **Entry points**: the user menu "Settings" item and the dashboard sidebar "Settings" item open the dialog in place instead of navigating. The user menu keeps its "Dashboard" item — it is the only route into the app from public pages on phones, where the navbar links and the dashboard sidebar are hidden.
...
Not in scope: new settings (notifications, email change), moving pro/player profile editing into the dialog, redesigning the user menu.
