...
13. `add-production-deploy` (in progress) — k3s single-node deployment on the netcup VPS (staging today): Traefik ingress + cert-manager, in-cluster PostgreSQL with nightly dumps to object storage, Prisma migration Job, deploy scripts from the operator's Mac; Jitsi tasks superseded by change 14
14. `replace-jitsi-with-livekit` (in progress) — self-hosted LiveKit replaces the Jitsi iframe: `VideoProvider` mints per-participant tokens, native call UI with pre-join, LiveKit webhooks stamp `connectedAt`/`leftAt` on attendance, one media-server process (compose in dev, host-network Deployment in k3s with embedded TURN); dev call verified over ICE/TCP; cluster verification (browser matrix, TURN fallback, resource audit) pending
